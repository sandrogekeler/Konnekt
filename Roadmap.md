...
- **Performance hardening** — TPS RCON poll interval 5s → 15s with server-flavor caching (Paper/Forge/Vanilla detected once, then one command per cycle instead of three); console log lines batched into 150ms flush windows to cap re-renders at ≤7/sec; Stats tile poll aligned to backend 10s sample rate; `RAMUsedMB` reuses cached gopsutil process handle.
- **Console filter toolbar** — collapsible (collapsed by default, shows active filter label); All/Warn/Error level filter + text search with inline match highlighting; match count.
- **Notifications filter** — collapsible All/Joins/Info/Warn/Errors segmented filter; Errors groups `crash` + `error`; per-filter empty states.
